...
fn main() {
    ...
    let rng_seed: u64 = SystemTime::now()
    ...
    if game.players.len() == 2 {
        ...
        if game.map.width == 40 {
            ...
            dropoff_turn = 143;
            reserve_dropoff_halite = 0;
            dropoff_distance_penalty = 0;
            dropoff_group_send = 0;
            search_radius = 1;
            random_death_turn = 13;
            exploring_move_multiplier = 142;
            stop_producing_ships_turn = 158;
        }
        ...
        if game.map.width == 56 {
            filter_divider = 2;
            dropoff_turn = 273;
            reserve_dropoff_halite = 0;
            dropoff_distance_penalty = 367;
            dropoff_group_send = 0;
            search_radius = 1;
            random_death_turn = 18;
            exploring_move_multiplier = 148;
            stop_producing_ships_turn = 188;
        }
    ...
    if game.players.len() == 4 {
        ...
        if game.map.width == 40 {
            filter_divider = 1;
            dropoff_turn = 236;
            reserve_dropoff_halite = 12;
            dropoff_distance_penalty = 55;
            dropoff_group_send = 1;
            search_radius = 0;
            random_death_turn = 19;
            exploring_move_multiplier = 143;
            stop_producing_ships_turn = 224;
        }
        if game.map.width == 48 {
            filter_divider = 2;
            dropoff_turn = 173;
        ...
        if game.map.width == 56 {
            ...
            dropoff_turn = 258;
            reserve_dropoff_halite = 0;
            dropoff_distance_penalty = 19;
            dropoff_group_send = 9;
            search_radius = 0;
            random_death_turn = 14;
            exploring_move_multiplier = 147;
            stop_producing_ships_turn = 193;
        }
        if game.map.width == 64 {
            filter_divider = 3;
            dropoff_turn = 132;
    ...
    for map_x in 0..game.map.width {
        for map_y in 0..game.map.height {
            ...
            let distance_penalty = dropoff_distance_penalty
                * game.map.calculate_distance(
    ...
    possible_cells_list = possible_cells_list
        .into_iter()
        .filter(|x| {
            for player in &game.players {
                if game.map.calculate_distance(
    ...
    Log::log(&format!(
        "Map possibilities calculated in:  {}.",
        search_start.elapsed().as_secs() as f64
    ...
    loop {
        ...
        for ship in &game.ships {
            if ship.1.position.x == me.shipyard.position.x
                && ship.1.position.y == me.shipyard.position.y
                && ship.1.owner != game.my_id
            {
        ...
        if dropoff_creating == 1 {
            Log::log(&format!(
                "Dropoff ship info: {:?}.",
                game.ships[&ship_id_for_dropoff]
            ));
        }
        ...
        if game.turn_number > dropoff_turn && dropoff_creating == 0 && possible_cells_list.len() > 0
        {
            for ship_id in &me.ship_ids {
                ...
                let possible_distance = map.calculate_distance(
                    &ship.position,
                    &Position {
                        x: possible_cells_list[0][0] as i32,
                        y: possible_cells_list[0][1] as i32,
                    },
                );
            ...
            if !ship_status.contains_key(&ship_id_for_dropoff) {
                ...
                ship_status
                    .entry(ship_id_for_dropoff)
                    .or_insert(ShipStates::SettlingDropoff);
            }
            dropoff_creating = 1;
        }
        ...
        for ship_id in &me.ship_ids {
            ...
            if dropoff_creating == 2
                && dropoff_group_send > 1
                && ship_status[ship_id] != ShipStates::GoTo
                && !ships_went_to_dropoff.contains(&ship_id.0)
            {
            ...
            if ship_status[ship_id] != ShipStates::Returning
                && ship_status[ship_id] != ShipStates::SettlingDropoff
                && ship_status[ship_id] != ShipStates::GoTo
                && ship.halite >= ((game.constants.max_halite as f32) * return_minimum) as usize
            {
                Log::log(&format!(
                    "Ship {} have more than 980 halite, returning",
                    ship_id.0
                ));
                ship_status.remove(&ship_id);
            ...
            if ship_status[ship_id] == ShipStates::GoTo {
                ...
                for dropoff_id in &me.dropoff_ids {
                    let distance_to_dropoff =
                        map.calculate_distance(&ship.position, &game.dropoffs[dropoff_id].position);
                ...
                if closest_distance < 5 {
                    Log::log(&format!(
                        "Ship {} already got close to dropoff {}",
                        ship_id.0, closest_id.0
                    ));
                    ship_status.remove(&ship_id);
                    ship_status.entry(*ship_id).or_insert(ShipStates::Exploring);
                }
            ...
            if ship_status[ship_id] != ShipStates::Returning
                && ship_status[ship_id] != ShipStates::SettlingDropoff
                && ship_status[ship_id] != ShipStates::GoTo
                && cell.halite > (game.constants.max_halite / divider) as usize
            {
                ship_status.remove(&ship_id);
            ...
            let command = match ship_status[ship_id] {
                ShipStates::Returning => {
                    ...
                    let mut closest_id = hlt::DropoffId(0);
                    for dropoff_id in &me.dropoff_ids {
                        let distance_to_dropoff = map.calculate_distance(
                            &ship.position,
                            &game.dropoffs[dropoff_id].position,
                        );
                        if distance_to_dropoff <= closest_distance {
                    ...
                    let distance_to_shipyard =
                        map.calculate_distance(&ship.position, &me.shipyard.position);
                ...
                ShipStates::Exploring => {
                    ...
                    let distance_to_shipyard =
                        map.calculate_distance(&ship.position, &me.shipyard.position);
                    if (distance_to_shipyard < 2 || closest_distance < 2)
                        && ((safe_moves.len() < 4 && safe_moves.len() > 1) || max_halite == 0)
                    {
                        let target_pos = if safe_moves.len() > 1 {
                            ship.position
                                .directional_offset(safe_moves[rng.gen_range(0, safe_moves.len())])
                ...
                ShipStates::Mining => ship.stay_still(),
                ShipStates::SettlingDropoff => {
                    let direction = navi.naive_navigate(
                        &ship,
                        &Position {
                            x: possible_cells_list[0][0] as i32,
                            y: possible_cells_list[0][1] as i32,
                    ...
                    if direction == Direction::Still
                        && (me.halite + &cell.halite + ship.halite) > 5000
                        && !cell_free.clone()
                    {
                        dropoff_creating = 2;
                        ship.make_dropoff()
        ...
        if (game_length - game.turn_number) <= random_death_turn {
            //destroy ships
        ...
        if (game_length - game.turn_number) >= stop_producing_ships_turn
